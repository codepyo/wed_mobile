...
function EventEntryForm({ current, onSubmit }: EntryProps) {
  ...
  return (
    <div className="event-entry">
      ...
      <p className="event-entry__intro">입장 정보는 이 브라우저에만 저장되며, 이벤트 화면의 개인화와 진행도에 사용됩니다.</p>
...
function completedTaskCount(session: LocalEventState) {
  return [session.cheerCount >= 5, session.scratchDone, session.fortuneIndex !== null, session.photoPassDone].filter(Boolean).length;
}
...
export function WeddingEvent({ phase, canEnter, preview = false }: Props) {
  ...
  const updateSession = (updater: (current: LocalEventState) => LocalEventState) => {
    setSession((current) => {
    ...
    <div className="wedding-event-layer" role="dialog" aria-modal="true" aria-label="Halloween Wedding Event">
      <div className="wedding-event-shell" ref={dialogRef}>
        ...
        {!session || editingEntry ? (
          <EventEntryForm current={session} onSubmit={setEntry} />
        ) : (
          <>
            ...
            <section className="event-card" id="event-photo-pass">
              <div className="event-section-head"><small>04 · HALLOWEEN PHOTO PASS</small><h3>오늘의 참석 인증 사진을 만들어보세요.</h3><p>사진은 서버에 올라가지 않습니다. 이 브라우저에서만 프레임을 합성해 바로 저장합니다.</p></div>
