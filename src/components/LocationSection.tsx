...
export function LocationSection({ onCopyAddress }: Props) {
  ...
  return (
    <section className="location-section" id="location" data-reveal>
      ...
      <KakaoMap latitude={wedding.ceremony.latitude} longitude={wedding.ceremony.longitude} venue={wedding.ceremony.venue} />
      ...
      <div className="transport-list">
        {wedding.transport.map((item, index) => <article className="transport-card" key={item.key}>
          <div className="transport-card__index">{String(index + 1).padStart(2, '0')}</div>
          <div className="transport-card__body"><small>{item.label}</small><h3>{item.title}</h3>{item.lines.map((line) => <p key={line}>{line}</p>)}</div>
        </article>)}
      </div>
    </section>
  );
